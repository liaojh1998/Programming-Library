...
public:
	SuffixArray(T* a, size_t n, int inc){
		N = n;
		incre = inc;
		arr = new int[N+3];
		for(int i = 0; i < N; i++)
	...
	void exec(){
		skewAlgorithm(arr, SAf, N, incre+100+1);
	}
};
...
void FastIO::solve(){
	...
	while(T--){
		...
		min = min < 0 ? -min : 0;
		SuffixArray<int> sa(diff, N-1, min);
		sa.exec();
		LongestCommonPrefix<int> lcp(sa.getArr(), sa.getSA(), N-1);
		lcp.exec();
		long long sum = 0, size = N-1;
		for(int i = 0; i < N-1; i++)
			sum += lcp[i];
		//Distinct number of subarrays problems:
		//Suffix array gives LCP
		//LCP gives repeated prefixes of all suffixes
		//Repeated prefixes combined with unrepeated suffixes = distinct number of subarrays
		//For example:
		//ABCDE
		//if AB has been repeated before,
		//we can make 3 more distinct subarrays:
		//ABC, ABCD, ABCDE
		//Which can be represented: Length of suffix - length of repeated prefix = number of new distinct subarrays
		//Total number of distinct subarrays = sum of all (Length of suffix - length of repeated prefix)
		//Note that length of a suffix = Array length - suffix beginning position + 1
		//There are 1 of all possible length of suffix, from 1 to Array length, so we can sum all of those with (array length)*(array length+1)/2
		//Total number of distinct subarrays = sum of all (length of suffix) - sum of all (length of repeated prefix)
		//Which = (Array length)*(Array length + 1)/2 - sum of LCP
		printf("%lld\n", (size*(size+1)/2-sum)%1000000009);
	}
}