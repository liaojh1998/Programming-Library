	...
	T query(int a, int b){
		...
		while(head[a] != head[b]){
			if(depth[head[a]] > depth[head[b]]){
				max = queryfunc(max, sgt->query(id[head[a]], id[a]+1));
				a = parent[head[a]];
			}else{
				max = queryfunc(max, sgt->query(id[head[b]], id[b]+1));
				b = parent[head[b]];
			}
		}
		if(depth[a] < depth[b])
			max = queryfunc(max, sgt->query(id[a]+1, id[b]+1));
		else
			max = queryfunc(max, sgt->query(id[b]+1, id[a]+1));
		return max;
	}
	...
	while(T--){
		...
		while(str[0] != 'D'){
			if(str[0] == 'Q'){
				...
				a--;
				readInt(b);
				if(hld.getDepth(u[a]) > hld.getDepth(v[a]))
					hld.update(u[a], b);
				else
					hld.update(v[a], b);
			}
